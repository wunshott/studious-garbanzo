...
#@export_range var: float(2.5, 4.5) var base_pitch := 3.5

@export_range(2.5, 4.5) var base_pitch: float
...
#	connect("finished", self, "play_next_sound")
	self.finished.connect(play_next_sound)
...
func _on_rich_text_label_voiceboxtext(input_text) -> void:
	play_string(input_text)
